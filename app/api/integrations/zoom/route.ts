...
export async function GET() {
  ...
  const integration = await one<{
    ...
    `SELECT id, config, enabled, created_at
     FROM integrations WHERE user_id = $1 AND provider = 'zoom'`,
    [g.user.id]
  )
...
export async function POST(req: Request) {
  ...
  const { access_token, refresh_token, meeting_default } = body as {
    access_token?: string
    refresh_token?: string
    meeting_default?: Record<string, unknown>
  }

  const config: Record<string, unknown> = {}
  if (access_token) config.access_token = access_token
  if (refresh_token) config.refresh_token = refresh_token
  if (meeting_default) config.meeting_default = meeting_default
...
export async function PATCH(req: Request) {
  ...
  await query(
    `UPDATE integrations SET enabled = $1, updated_at = now()
     WHERE user_id = $2 AND provider = 'zoom'`,
    [enabled, g.user.id]
  )
...
export async function DELETE(req: Request) {
  ...
  await query(
    `DELETE FROM integrations WHERE user_id = $1 AND provider = 'zoom'`,
    [g.user.id]
  )
  return NextResponse.json({ ok: true })
}
